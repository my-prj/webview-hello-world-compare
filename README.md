...
| Saucer | https://github.com/saucer/saucer | 336 KB local, CI pending | C++23, Objective-C++ | saucer 8.0.5 + 11 CPM git deps; vendored CMake 4.4.2 + Ninja 1.13.2 (~418M under `temp/saucer/`) | T1=302s, T2=240s (build.sh 16s), T3≈2040s* (session interrupted, includes GHA queue — inaccurate), T4 pending; local `.app` 336 KB; CI pending; [CI run](https://github.com/my-prj/webview-hello-world-compare/actions/runs/31960312213) (*T3 not reliable) |
| WebUI | https://github.com/webui-dev/webui | 220 KB local, CI pending | C++17 | webui 2.5.0-beta.3 (static lib); ~14M under `temp/webui/` (no vendored CMake/Ninja) | T1=41s, T2=480s, T3 pending, T4 pending; local `.app` 220 KB; CI pending |
